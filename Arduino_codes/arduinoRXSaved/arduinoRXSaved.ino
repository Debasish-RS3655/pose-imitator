...
void setup() {
  ...
  Serial3.begin(9600);        //communicating with the Pro Mini
  //Serial.print("Servo controller ready.");
  //Serial.println("\n");
  pinMode(LED_BUILTIN, OUTPUT);
  servoHeadHor.attach(12);
  servoHeadVer.attach(32);
  servorightShoulderY.attach(10);
  servoleftShoulderY.attach(9);
  servorightShoulderX.attach(8);
  servoleftShoulderX.attach(30);
...
void loop() {
  if (Serial3.available()) {
    ...
    if (inputText.length() > 16) inputText.remove(16);
    //Serial.println("");
